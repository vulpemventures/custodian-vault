...
func TestTransaction(t *testing.T) {
	...
	t.Run("Sign transaction for BIP44 wallet", func(t *testing.T) {
		...
		resp, err := newSignature(t, b, storage, name, rawTx, mode, token)
		if err != nil {
			t.Fatal(err)
		}
		if resp == nil {
	...
	t.Run("Sign transaction with expired token should fail", func(t *testing.T) {
		...
		_, err := newSignature(t, b, storage, name, rawTx, mode, token)
		if err == nil {
...
func newSignature(t *testing.T, b logical.Backend, store logical.Storage, name string, rawTx string, mode string, token string) (*logical.Response, error) {
